...
LAB_INTEGRATION_WORKSPACE_ENV = "AUTORESEARCH_MLX_LAB_INTEGRATION_WORKSPACE"
LAB_INTEGRATION_TARGET_ENV = "AUTORESEARCH_MLX_LAB_INTEGRATION_TARGET"
...
def integration_environment(*, workspace: Path) -> dict[str, str]:
    ...
    if not supports_direct_integration(target):
        ...
        raise ValueError(
            f"Target {target!r} does not yet support direct MLX training integration. "
...
def current_integration_workspace() -> IntegrationWorkspace | None:
    ...
    if not supports_direct_integration(metadata_target):
        return None
    spec = importlib.util.spec_from_file_location("autoresearch_mlx_lab_integration_kernel", kernel_path)
...
def active_integration_target() -> str | None:
    workspace = current_integration_workspace()
    return None if workspace is None else workspace.target
