...
Use `python3 tools/changelog_scores.py --group-by day --format csv` when you want a plotting-friendly daily autonomy summary, `--group-by subsystem --format csv` when you want subsystem totals, `--group-by day-subsystem --format csv` when you want daily subsystem slices, or `--group-by entry --verify` to sanity-check header totals and enforce scoped headers.
