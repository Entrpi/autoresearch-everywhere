...
- For kernel-lab work, treat `profile` / `extract` / `orchestrate` as the heuristic layer, the ledger in `results/kernel_lab/ledger.jsonl` as the memory of what has already been proved, and `capture` as the trace-backed truth layer. Microbench wins are enough to justify trying a target; use a `.gputrace` before making stronger Apple Silicon performance claims when practical, feed that capture back into `orchestrate --trace-metadata ...`, and prefer promotion-ready workspaces over opening redundant fresh ones.
