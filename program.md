...
- For kernel-lab work, treat `profile` / `extract` / `orchestrate` as the heuristic layer and `capture` as the trace-backed truth layer. Microbench wins are enough to justify trying a target; use a `.gputrace` before making stronger Apple Silicon performance claims when practical, and feed that capture back into `orchestrate --trace-metadata ...` once a target graduates from "candidate" to "worth serious investigation."
