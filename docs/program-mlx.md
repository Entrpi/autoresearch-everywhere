...
- runtime-path changes such as compilation strategy, kernel choice, batching logic, cache behavior, or checkpointing behavior that may shift throughput or pressure across preset families
- evaluator changes that alter sequence length, slicing, token budgets, or canonical/proxy semantics
- optimizer or training-loop changes that alter accumulation, state shape, step timing, or memory pressure
...
When you rely on the runtime eval selector, read and report the selector provenance from the run summary: status, effective confidence, freshness, telemetry count, commit/day spread, stable-rung coverage, last-seen date, and any limit reason. Those fields are the first-line semantic-drift guard for preset and hardware coverage. Use `./.venv/bin/python tools/calibrate_eval_policy.py telemetry-summary --preset <preset>` when you need to inspect the passive evidence base directly.
