...
| Resume/checkpoint support | Not present | Exact full-state or approximate weights-only checkpointing in `train_mlx.py` | Improvement | The MLX path now supports exact step-boundary resume with model, optimizer, runtime counters, and train-loader state, plus a lighter weights-only mode that restores only model weights and resumes approximately. The end-of-run summary also separates current-invocation timing from cumulative progress so resumed reports stay scope-consistent. |
